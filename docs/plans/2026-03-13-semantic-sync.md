...
Vync diff 파이프라인이 구조적 변경("Moved: B — root → A")은 감지하지만, 사용자의 의미적 의도("B는 A에 종속되는 개념")를 안정적으로 추론하지 못함. D-017 설계 문서(`docs/plans/2026-03-13-semantic-sync-design.md`)에서 해석 레벨 체계, 시각화 유형별 규칙, 구조화된 반환 포맷, 모호성 처리 전략을 정의함. 이 계획은 해당 설계를 구현.

...
- `init <file>` — Create an empty .vync canvas file in CWD/.vync/. Appends .vync extension if missing.
...
For `read`: use the Read tool to read the .vync file (bare filename resolves to `.vync/<file>.vync`), then present a structured summary:
- Total element count

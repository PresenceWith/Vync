...
import { buildBoardHotkeyPlugin } from './plugins/with-hotkey';
...
export const VyncBoard: React.FC<BoardProps> = ({
  value,
  viewport,
  theme,
  onChange,
  ...
  const [appState, setAppState] = useState<BoardState>(() => {
  ...
  const [board, setBoard] = useState<BoardWithState | null>(null);

  if (board) {
    board.appState = appState;
  }

  const updateAppState = (newAppState: Partial<BoardState>) => {
    setAppState({
      ...appState,
      ...newAppState,
    });
  ...
  const plugins: PlaitPlugin[] = [
    ...
    withMindExtend,
    withCommonPlugin,
    buildBoardHotkeyPlugin(updateAppState),
    withFreehand,
  ...
  return (
    <I18nProvider>
      <BoardContext.Provider value={{ appState, setAppState }}>
        <div
          className={classNames('vync-board', {
            'vync-board--mobile': appState.isMobile,
          })}
          ref={containerRef}
        >
          <Wrapper
          ...
          >
            <Board
              afterInit={(board) => {
                setBoard(board as BoardWithState);
